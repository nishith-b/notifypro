...
export default function Home() {
  const handleSendNotification = async () => {
    const sendingToast = toast.loading("Sending notification...");

    try {
      const res = await axios.post(
        `${import.meta.env.VITE_API_URL}/api/notification/send`,
        { message: "Hello from NotifyPro!" },
        { withCredentials: true }
      );

      toast.success("Notification sent!", { id: sendingToast });
      console.log("Notification response:", res.data);
    } catch (err) {
      console.error("Error sending notification:", err);
      toast.error("Failed to send notification", { id: sendingToast });
    }
  };
